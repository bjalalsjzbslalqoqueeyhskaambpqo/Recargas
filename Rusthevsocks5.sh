...
async fn kick_api(s: SessionMap, w: WaitRoom) {
    ...
    let app = Router::new()
        ...
        .route("/promote", get(|State(st): State<InternalState>, Query(p): Query<HashMap<String, String>>| async move { if let Some(id) = p.get("id").filter(|i| !i.is_empty()) { if let Some((_, tx)) = st.waitroom.remove(id.as_str()) { let _ = tx.send(()); "promoted".to_string() } else { "not_waiting" } } else { "missing_id" }.to_string() }))
