    ...
    let t_h2c = tokio::spawn(async move {
        ...
        loop {
            if let Ok(Ok(n)) = time::timeout(STREAM_IDLE_TIMEOUT, hr.read(&mut buf)).await {
                ...
                tokio::task::yield_now().await; // QoS MAGICO: Fuerza ceder el paso a paquetes pequeños (WhatsApp)
            } else { break; }
        }
    });
...
async fn kick_api(s: SessionMap, w: WaitRoom) {
    ...
    let app = Router::new()
        .route("/kick", get(|State(st): State<InternalState>, Query(p): Query<HashMap<String, String>>| async move { if let Some(id) = p.get("id").filter(|i| !i.is_empty()) { if kick_session(&st.sessions, id, if p.get("reason").map(|s|s.as_str())==Some("expired") {T_EXPIRED} else {T_KICK}) { "kicked" } else { "not_connected" } } else { "missing_id" }.into() }))
        ...
        .route("/promote", get(|State(st): State<InternalState>, Query(p): Query<HashMap<String, String>>| async move { if let Some(id) = p.get("id").filter(|i| !i.is_empty()) { if let Some((_, tx)) = st.waitroom.remove(id.as_str()) { let _ = tx.send(()); "promoted" } else { "not_waiting" } } else { "missing_id" }.into() }))
...
impl AppState {
    async fn new() -> Self { Self { token: Arc::new(fs::read_to_string(TOKEN_PATH).await.unwrap_or_default().trim().into()), mu: Arc::new(Mutex::new(())), rate: Arc::new(std::sync::Mutex::new(HashMap::new())) } }
...
async fn main() -> Result<()> {
    ...
    let app = Router::new()
        ...
        .route("/client", get(|State(st): State<AppState>, h: HeaderMap, ConnectInfo(a): ConnectInfo<SocketAddr>, Query(p): Query<HashMap<String, String>>| async move { if let Some(e) = st.ok(&a.ip().to_string(), &h) { return e; } let id = p.get("id").unwrap_or(&String::new()).trim(); if id.is_empty() { return (StatusCode::BAD_REQUEST, Json(serde_json::json!({"error":"falta id"}))); } let _l = st.mu.lock().await; if let Some(u) = load().await.get(id) { (StatusCode::OK, Json(row(id, u))) } else { (StatusCode::NOT_FOUND, Json(serde_json::json!({"error":"no encontrado"}))) } }))
        .route("/client/create", post(|State(st): State<AppState>, h: HeaderMap, ConnectInfo(a): ConnectInfo<SocketAddr>, Json(b): Json<CReq>| async move { if let Some(e) = st.ok(&a.ip().to_string(), &h) { return e; } let id = b.id.trim(); if id.is_empty() { return (StatusCode::BAD_REQUEST, Json(serde_json::json!({"error":"falta id"}))); } let _l = st.mu.lock().await; let mut u = load().await; if u.contains_key(id) { return (StatusCode::CONFLICT, Json(serde_json::json!({"error":"ya existe"}))); } let nm = b.name.unwrap_or_default().trim().to_string(); let nm = if nm.is_empty() { "sin-nombre".into() } else { nm }; let usr = User { name: nm, expires_ts: now_secs() + b.days.unwrap_or(30).max(0) * 86400 }; u.insert(id.into(), usr.clone()); save(&u).await; tokio::spawn(call(format!("{PROMOTE_BASE}{id}"))); (StatusCode::CREATED, Json(row(id, &usr))) }))
        ...
        .route("/client/update", put(|State(st): State<AppState>, h: HeaderMap, ConnectInfo(a): ConnectInfo<SocketAddr>, Json(b): Json<UReq>| async move { if let Some(e) = st.ok(&a.ip().to_string(), &h) { return e; } let id = b.id.trim(); if id.is_empty() { return (StatusCode::BAD_REQUEST, Json(serde_json::json!({"error":"falta id"}))); } let (fid, usr, ko) = { let _l = st.mu.lock().await; let mut u = load().await; let Some(mut usr) = u.get(id).cloned() else { return (StatusCode::NOT_FOUND, Json(serde_json::json!({"error":"no encontrado"}))); }; if let Some(n) = b.name.as_deref().map(str::trim).filter(|s| !s.is_empty()) { usr.name = n.into(); } if let Some(d) = b.days { usr.expires_ts = now_secs() + d.max(0) * 86400; } let (fid, ko) = match b.new_id.as_deref().map(str::trim).filter(|s| !s.is_empty() && *s != id) { Some(nid) => { u.remove(id); (nid.into(), true) }, None => (id.into(), false) }; u.insert(fid.clone(), usr.clone()); save(&u).await; (fid, usr, ko) }; if ko { tokio::spawn(call(format!("{KICK_BASE}{id}&reason=kicked"))); } if usr.expires_ts <= now_secs() { tokio::spawn(call(format!("{KICK_BASE}{fid}&reason=expired"))); } else { tokio::spawn(call(format!("{PROMOTE_BASE}{fid}"))); } (StatusCode::OK, Json(row(&fid, &usr))) }))
