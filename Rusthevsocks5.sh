...
type Sessions = Arc<DashMap<String, (u64, mpsc::Sender)>>;
...
async fn proxy_tcp(mut req: h2::RecvStream, mut resp_tx: h2::SendStream, authority: String) {
                    ...
                    while !chunk.is_empty() {
                        ...
                        match std::future::poll_fn(|cx| resp_tx.poll_capacity(cx)).await {
                            Some(Ok(cap)) if cap > 0 => {
                                ...
                                if resp_tx.send_data(data, false).is_err() {
                                    return;
                                }
                            }
                            _ => return,
...
async fn proxy_udp(mut req: h2::RecvStream, mut resp_tx: h2::SendStream, authority: String) {
    let Ok(udp) = UdpSocket::bind("0.0.0.0:0").await else {
        let _ = resp_tx.send_reset(h2::Reason::CONNECT_ERROR);
        return;
    };
    if time::timeout(Duration::from_secs(5), udp.connect(&authority)).await.is_err() {
        let _ = resp_tx.send_reset(h2::Reason::CONNECT_ERROR);
        return;
    }
    let udp = Arc::new(udp);
    ...
    let t_up = tokio::spawn(async move {
        ...
        while let Some(Ok(chunk)) = req.data().await {
            ...
            while buf.len() >= 2 {
                ...
                buf.advance(2);
                if u_tx.send(&buf[..len]).await.is_err() {
                    return;
                }
                buf.advance(len);
            }
            ...
            while !data.is_empty() {
                ...
                match std::future::poll_fn(|cx| resp_tx.poll_capacity(cx)).await {
                    Some(Ok(cap)) if cap > 0 => {
                        ...
                        if resp_tx.send_data(to_send, false).is_err() {
                            return;
                        }
                    }
                    _ => return,
...
async fn expiration_reaper(sessions: Sessions, cache: AuthCache) {
    loop {
        ...
        let now = now_secs();
        let to_kick: Vec<String> = sessions
            .iter()
            .filter_map(|kv| {
                let id = kv.key();
                if let Some(entry) = cache.get(id) {
    ...
    loop {
        ...
        tokio::spawn(async move {
            ...
            if line.starts_with("GET /kick?") {
                ...
                let reason = find_param(line, "reason=");
                let kicked = if let Some(entry) = sess.get(id.unwrap_or("")) {
                    let _ = entry.value().1.send(format!("KICK:{}", reason.unwrap_or("kicked"))).await;
                    true
                } else {
                    false
                };
                ...
                let ids: Vec<String> = sess.iter().map(|kv| kv.key().clone()).collect();
                ...
                let id = find_param(line, "id=");
                let promoted = if let Some(entry) = sess.get(id.unwrap_or("")) {
                    let _ = entry.value().1.send("PROMOTE".to_string()).await;
                    true
                } else {
                    false
                };
...
fn find_param<'a>(line: &'a str, p: &str) -> Option<&'a str> {
    line.split('?')
        .nth(1)?
        .split('&')
...
async fn main() -> Result<()> {
    ...
    loop {
        if let Ok((c, _)) = listener.accept().await {
            tokio::spawn(handle_conn(c, sess.clone(), cache.clone()));
        }
    }
}
...
impl AppState {
    fn new() -> Self {
        ...
        Self {
            ...
            rate: Arc::new(std::sync::Mutex::new(HashMap::new())),
        }
    }
...
fn load_users_blocking() -> HashMap<String, User> {
    let Ok(c) = std::fs::read_to_string(USERS_PATH) else {
        return HashMap::new();
    };
    ...
    for line in c.lines() {
        ...
        let Some(exp) = parts.next().map(str::trim) else {
            continue;
        };
...
async fn load_users() -> HashMap<String, User> {
    tokio::task::spawn_blocking(load_users_blocking)
        .await
        .unwrap_or_default()
}
...
async fn reload_proxy() {
    if let Ok(c) = reqwest::Client::builder()
        ...
        .build()
    {
        let _ = c.get(RELOAD_URL).send().await;
    }
}
...
async fn handle_clients(
    ...
    if let Some(e) = auth_check(&st, &headers, &addr) {
        return e;
    }
    ...
    (
        StatusCode::OK,
        Json(serde_json::json!({"clients": rows, "total": rows.len()})),
    )
}
...
async fn handle_client(
    ...
    let _l = st.users_mu.lock().await;
    match load_users().await.get(&id).cloned() {
...
async fn handle_create(
    ...
    if id.is_empty() {
        return err_resp(StatusCode::BAD_REQUEST, "falta id");
    }
...
async fn handle_update(
    ...
    let (final_id, u, kick_old) = {
        ...
        if let Some(p) = body.phone.as_deref() {
            u.phone = p.trim().to_string();
        }
